...
function params = params_battery_calce_slow(params_sim)
    ...
    % params.opt_vars = [1 10 14 18 22];
    params.opt_vars = [2 8:10 12:14 16:18 20:22];
...
%         params.X(traj).val(params.multi_traj_var,1) = params.X(traj-1).val(params.multi_traj_var,1) + abs(params.X(traj-1).val(params.multi_traj_var,1)).*[1.5; 0.5; 1.5; 0.5];
    ...
    originalIndices = linspace(1, length(params_sim.input_data.SOC), length(params_sim.input_data.SOC));
    
    % Target indices for resampled data, aiming for 250 points
    targetIndices = linspace(1, length(params_sim.input_data.SOC), 1000);
    
    % Resampled SOC using Piecewise Cubic Hermite Interpolating Polynomial (PCHIP)
    params.params_GT_sample = interp1(originalIndices, params_sim.input_data.SOC, targetIndices, 'pchip');
